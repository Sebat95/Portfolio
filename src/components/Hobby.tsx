...
const Hobby = (props: HobbyProps) => {
    ...
    return (
        <group {...props} dispose={null} position={props.position} ref={props.innerRef}
            onPointerEnter={() => setHover(!props.highlighted)} onPointerLeave={() => setHover(false)}>
                {
                    React.cloneElement(React.Children.only(props.children) as JSX.Element, {...childProps})
                }
        </group>
    )
}
