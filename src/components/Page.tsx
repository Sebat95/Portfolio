...
const Page = (prop: PageProp) => {
  ...
  const pageSkinnedMesh = useMemo(() => {
    ...
    const materials = [
      ...
      new MeshStandardMaterial({
        color: white,
        map: frontMap,
        ...(pageNum === 0 ? { roughnessMap: roughMap } : { roughness: 1 }),
  ...
  useFrame((_, delta) => {
    if (
      ref.current &&
      pageMeshRef.current &&
      Array.isArray(pageMeshRef.current.material)
    ) {
      const emissiveIntensity = highlighted ? 0.22 : 0;
      (
        pageMeshRef.current.material[4] as MeshStandardMaterial
      ...
      (
        pageMeshRef.current.material[5] as MeshStandardMaterial
      ).emissiveIntensity = MathUtils.lerp(
        (pageMeshRef.current.material[5] as MeshStandardMaterial)
          .emissiveIntensity,
        emissiveIntensity,
  ...
  return (
    <group
      {...prop}
      ref={ref as RefObject<Group<Object3DEventMap>>}
      onPointerEnter={(e) => {
        e.stopPropagation();
        setHighlighted(true);
      }}
